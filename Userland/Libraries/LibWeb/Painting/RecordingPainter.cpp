...
struct CommandExecutionState {
    CommandExecutionState(AccelGfx::Canvas& canvas)
        : painter(AccelGfx::Painter(canvas))
    {
    }

    AccelGfx::Painter painter;
};
...
CommandResult DrawTextRun::execute(CommandExecutionState& state) const
{
    ...
    state.painter.draw_text_run(baseline_start.to_type<float>(), Utf8View(string), font, color);
    return CommandResult::Continue;
}
...
CommandResult FillRect::execute(CommandExecutionState& state) const
{
    if (color.alpha() == 0)
        return CommandResult::Continue;
    // (void)state;
    dbgln("> FillRect::execute rect=({}) color=({})", rect, color);
    state.painter.fill_rect(rect, color);
    return CommandResult::Continue;
...
void RecordingPainter::execute(Gfx::Bitmap& bitmap)
{
    ...
    state.painter.prepare_glyph_atlas(unique_glyphs_per_font);

    for (auto& command : m_painting_commands) {
        command.visit([&](auto const& command) { return command.execute(state); });
    }

    canvas.flush();

    Gfx::Painter target_painter(bitmap);
    target_painter.blit({ 0, 0 }, canvas.bitmap(), canvas.bitmap().rect());
