...
void Interpreter::do_return(Value value)
{
    reg(Register::return_value()) = value;
    reg(Register::exception()) = {};
}

void Interpreter::leave_execution_context()
{
    ...
    m_running_execution_context = &running_execution_context;
    m_current_executable = running_execution_context.executable;
    m_realm = GCPtr { vm().current_realm() };
    m_global_object = GCPtr { m_realm->global_object() };
    m_global_declarative_environment = GCPtr { m_realm->global_environment().declarative_record() };
    m_arguments = running_execution_context.arguments;
    m_registers_and_constants_and_locals = running_execution_context.registers_and_constants_and_locals.span();

    m_program_counter = running_execution_context.program_counter.value();
}
...
{
    //    dbgln(">>>>>>>Interpreter::run_bytecode pc=({:x})", m_program_counter);
    //    ScopeGuard guard = [&] {
    //        dbgln("<<<<<<<Interpreter::run_bytecode pc=({:x})", m_program_counter);
    //    };
    ...
    TemporaryChange change_depth { m_depth, (size_t)0 };
    ...
    //    size_t m_program_counter =entry_point;

    //    TemporaryChange change(m_program_counter, Optional<size_t&>(entry_point));
    //    dbgln(">set pc to {:x} executable=({})", entry_point, this);
    m_program_counter = entry_point;
...
#define DISPATCH_NEXT(name)                                                                           \
    ...
    start:
        ...
        handle_End: {
            ...
            if (m_depth > 0) {
                m_depth--;
                leave_execution_context();
                auto dst = m_running_execution_context->result_dst;
                if (dst.has_value()) {
                    set(*dst, js_undefined());
                    m_running_execution_context->result_dst = {};
                }
                goto start;
            } else {
                return;
