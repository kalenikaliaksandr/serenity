...
CodeGenerationErrorOr<void> Generator::emit_function_declaration_instantiation(ECMAScriptFunctionObject const& function)
{
    ...
    for (auto const& declaration : function.m_functions_to_initialize) {
        ...
        emit<Op::NewFunction>(dst, declaration, OptionalNone {});
        if (declaration.name_identifier()->is_local()) {
            emit<Op::Mov>(local(declaration.name_identifier()->local_variable_index()), dst);
        } else {
            emit<Op::SetVariable>(intern_identifier(declaration.name()), dst, next_environment_variable_cache(), Op::SetVariable::InitializationMode::Set, Op::EnvironmentMode::Var);
        }
    }
